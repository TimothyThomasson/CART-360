...
void setRGB()
{
  switch (mode)
  {
    ...
    case 4:
      {
        analogWrite(LED_PIN_R, 80);   // Turn on the LED -R
        analogWrite(LED_PIN_G, 0);   // Turn on the LED -G
        analogWrite(LED_PIN_B, 150);   // Turn on the LED -B
        break;
      }
    default:
      {
...
void live()
{
  ...
  if (analogRead(NOTE_IN_PIN) > 0) {
    tone(BUZZER_PIN, (analogRead(NOTE_IN_PIN) + offsetFrequency * 9) / 6, duration);
    delay(50);
...
**************************************************************************/
void startUpTimer()
//uses millis functin to get miliseconds since program starts
{
  startTime = millis();
...
**************************************************************************/
void updateTimer()
//gets current time passed by minusing running millis func total with startTime
{
...
{
//used to play current note using test note var. Adding offsetFrequency for photocell.
...
{
//updating array with note values and time passed numbers
  notes[countNotes] = activeFrequency;
  durations[countNotes] = timePassed;
  countNotes ++;
//releasing button and flipping boolean to false
//resetting time passed counter
...
{
//averaging changing values each time note is held for all 16 samples
  int sampleTotal = 0;
  int rawSenseVal = analogRead(offsetFrequency);
...
{
  //changing dimness of LED when called with inputted parameter
  analogWrite(LED_PIN_R, col);   // Turn on the LED -R
  analogWrite(LED_PIN_G, 0);   // Turn on the LED -G
  analogWrite(LED_PIN_B, 0);   // Turn on the LED -B
...
void playWithDuration()
{
  for (int i = 0; i < 16; i ++) {
    ...
    delay(durations[i]);
    //change dimness of LED (divided by 5 to make more noticable)
    colorLED(notes[i] / 5);
    delay(100);
    //when note is realsed break from loop
    if (digitalRead(BUTTON_MODE_PIN) == HIGH) {
      break;
    }
  }
